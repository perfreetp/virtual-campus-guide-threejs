...
- 智能路线规划：支持起点、终点与多个途经点，步行/骑行两种模式切换，自动优化途经顺序并计算最优路径，地面发光线条高亮，途经点依次标注名称与距离。
- 镜头沿路飞行：一键让镜头沿规划路径平滑飞行浏览，途经点依次高亮，面板同步显示当前站点。
- 实时人流热力：数字孪生热力图层按颜色深浅表现各楼栋人流密度，支持按院系和楼栋筛选，点击热区查看楼层、人数与预警信息。
- 个人中心：保存的路线、收藏的建筑与最近导览记录持久化到本地，刷新不丢失，历史记录可一键回放路线与镜头动画。
...
src/
  components/
    ...
    ControlPanel.vue      # 搜索与分类筛选
    RoutePlanner.vue      # 智能路线规划面板
    HeatmapPanel.vue      # 实时人流热力面板
    UserCenter.vue        # 个人中心（收藏/路线/历史）
    InfoPanel.vue         # 建筑信息面板
    StatsCards.vue        # 数据卡片
  mock/
    campusData.js         # 校园建筑与推荐路线数据
    crowdData.js          # 楼栋人流与院系数据
  styles/
  ...
  utils/
    pathfinding.js        # 多点路径规划、距离估算与小地图坐标
    storage.js            # 个人中心本地持久化
  App.vue
  main.js
```
